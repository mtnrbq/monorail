...
Target.create "Bundle" (fun _ ->
    [ "server", dotnet $"publish -c Release -o {distPath}" serverPath
      "client", dotnet $"fable -e .jsx -o build --run vite build --outDir {distPath}/public" clientPath ]
    |> runParallel
)
...
Target.create "Run" (fun _ ->
    [ "server", dotnet "watch run" serverPath
      "client", dotnet "fable watch -e .jsx -o build --run vite -c ../../vite.config.js" clientPath ]
    |> runParallel
)
...
Target.create "Client" (fun _ ->
    run dotnet $"fable watch -e .jsx -o build --run vite -c ../../vite.config.js" clientPath
)
...
Target.create "Test" (fun _ ->
    if System.IO.Directory.Exists testPath then
        [ "server", dotnet "run" (testPath + "/Server")
          "client", dotnet "fable -e .jsx -o build --run vite build" (testPath + "/Client") ]
        |> runParallel
    else ()
)
